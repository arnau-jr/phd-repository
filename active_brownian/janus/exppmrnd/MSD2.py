...
from formulas import Dex_exponential_f,Dex_plusminus_f,Dex_randomplusminus_a
...
data_msd = np.loadtxt('./result2Dexp2/MSD_lags.dat')
data_vel = np.loadtxt('./result2Dexp2/vel.dat')
data_velabs = np.loadtxt('./result2Dexp2/velabs.dat')
...
print(f'Defftheoryfourier = {(DT+Dex_randomplusminus_a(v,T,tau,1/DR,N=int(1e6))):.6f}')
...
plt.savefig('plots/vel2.pdf',format='pdf',bbox_inches='tight',pad_inches=0.02)
...
plt.ylabel(r'v ($\upmu\mathrm{m}/\mathrm{s}$)')
freq = 250
plt.plot(data_velabs[:,0],data_velabs[:,3],'g-')
plt.savefig('plots/velabs2.pdf',format='pdf',bbox_inches='tight',pad_inches=0.02)
