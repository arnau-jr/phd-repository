...
Deff = []
Derr = []
data_msd = np.loadtxt('./result2Dexp2_1/MSD_lags.dat')
data_msdavg = np.zeros([data_msd.shape[0],3])
for sample in range(1,Nsamples+1):
    data_msd = np.loadtxt(f'./result2Dexp2_{sample}/MSD_lags.dat')
...
print(f'Defftheory            = {(DT+DT*Pe2*f_randomplusminus_sa(T/tau,T*DR)):.6f}')
...
plt.plot(t_sample,4*(DT+DT*Pe2*f_randomplusminus_sa(T/tau,T*DR))*t_sample,'g-',label=r'Theo: $D_\mathrm{eff}t$')
...
plt.plot(t_sample,4*Deff.mean()*t_sample,'b-',label=r'$D_\mathrm{eff}t$')
plt.legend()
plt.savefig('plots/MSD2_short.pdf',format='pdf',bbox_inches='tight',pad_inches=0.02)
...
freq = 150
plt.plot(t_sample,4*(DT+DT*Pe2*f_randomplusminus_sa(T/tau,T*DR))*np.ones_like(t_sample),'g-',label=r'Theo: $D_\mathrm{eff}t$')
...
plt.vlines([40.0,80.0,120.0,160.0,200.0,240.0,280.0,320.0,360.0],plt.ylim()[0],plt.ylim()[1],linestyles='dashed',colors='black',linewidths=0.75,alpha=0.25)
plt.legend()
plt.savefig('plots/DER2.pdf',format='pdf',bbox_inches='tight',pad_inches=0.02)
# plt.show()
plt.close()
...
plt.plot(data_msdavg[::freq,0],np.gradient(data_msdavg[:,1],data_msdavg[:,0])[::freq],'rx',label='Data')
plt.legend()
plt.savefig('plots/DER2_short.pdf',format='pdf',bbox_inches='tight',pad_inches=0.02)
# plt.show()
plt.close()
