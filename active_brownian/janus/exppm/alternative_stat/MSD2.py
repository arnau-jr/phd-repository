...
print(f'Defftheoryfourier     = {(DT+DT*Pe2*f_plusminus_f(T/tau,T*DR,N=int(1e6))):.6f}')
...
plt.ylabel(r'MSD ($\upmu\mathrm{m}^2$)')
freq = 200
plt.plot(t_sample,4*(DT+DT*Pe2*f_plusminus_f(T/tau,T*DR,N=int(1e6)))*t_sample,'g-',label=r'Theo: $D_\mathrm{eff}t$')
...
freq = 150
plt.plot(t_sample,4*(DT+DT*Pe2*f_plusminus_f(T/tau,T*DR,N=int(1e6)))*np.ones_like(t_sample),'g-',label=r'Theo: $D_\mathrm{eff}t$')
