...
P_map = np.empty((Nt+1,Npro,Npro,natoms_central))
Pproj_map = np.empty((Nt+1,Npro,Npro,natoms_central))
CP_map = np.empty((Nt+1,Npro,Npro,natoms_central))
CPproj_map = np.empty((Nt+1,Npro,Npro,natoms_central))
...
if(master):
    ...
    f" {np.sum(P_map[0,:,:,:])-np.sum(P_to_solvent[0,:]):21.14E}\n"\
    )
...
for i in range(1,Nt):
    ...
    if(master):
        ...
        P_map[i,:,:,:] = power_map
        Pproj_map[i,:,:,:] = powerproj_map
        CP_map[i,:,:,:] = Cpower_map
        CPproj_map[i,:,:,:] = Cpowerproj_map
        ...
        if(i>1):
            ...
            work_map += P_map[i-1,:,:,:]*dt/2. + P_map[i,:,:,:]*dt/2.
            workproj_map += Pproj_map[i-1,:,:,:]*dt/2. + Pproj_map[i,:,:,:]*dt/2.
            Cwork_map += CP_map[i-1,:,:,:]*dt/2. + CP_map[i,:,:,:]*dt/2.
            Cworkproj_map += CPproj_map[i-1,:,:,:]*dt/2. + CPproj_map[i,:,:,:]*dt/2.
    ...
    if(i%step==0 and master):
        ...
        f" {np.sum(P_map[i,:,:,:])-np.sum(P_to_solvent[i,:]):21.14E}\n"\
        )
